...
zk.ComboBox = function(ops) {
	...
	var _setdropstyle = function(argRow) {
		...
		if (combo.imageable) {
			zk.addCss(cells[0], 'combobox_cell_drop_img');
		}
	};

	var _setStyle = function() {
		zk.addCss(dropdown, 'combobox_table_drop');
		zk.addCss(divdrop, 'combobox_drop_div');
		zk.addCss(toptable, 'combobox_table_top');
		zk.addCss(searchTxt, 'combobox_text_search');
	};
	...
	var _iniTr = function() {
		...
		zk.addCss(edit, 'combobox_input_text');
		zk.addCss(btn, 'combobox_btn');
		zk.addCss(toptr.cells[0], 'combobox_cell_img');
		zk.addCss(toptr.cells[1], 'combobox_cell_edt');
		zk.addCss(toptr.cells[2], 'combobox_cell_btn');
		if (topimg) {
